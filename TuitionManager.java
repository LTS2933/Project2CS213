            ...
            while (running) {
                ...
                switch (firstOperation) {
                        ...
                        for (int i = 0; i < this.enrollment.getSize(); i++) { //iterates through enrollment
                            for (int j = 0; j < this.roster.getSize(); j++) { //iterates through roster
                                ...
                                if (enroll.equals(roster)) {
                                    ...
                                    System.out.println(this.roster.returnRoster()[j].toString() + ": tuition due: $" + df.format(tuition));
                                }
                            }
                        }
                        break;
                    ...
                    case "SE":
                        ...
                        int arr[] = new int[10];
                        int i,j;
                        for (i = 0; i < this.enrollment.getSize(); i++) { //iterates through enrollment
                            for (j = 0; j < this.roster.getSize(); j++) { //iterates through roster
                                Profile enroll = this.enrollment.returnEnrollStudent()[i].getProfile(); //finds the profile through enrollment
                                Profile roster = this.roster.returnRoster()[j].getProfile(); //finds the profile through roster
                                if (enroll.equals(roster)) {
                                    int total = this.enrollment.returnEnrollStudent()[i].getCreditsEnrolled() + this.roster.returnRoster()[j].getCredits();
                                    if (total >= 120) {
                                        arr[i] = total;
                                    }
                                }
                            ...
                            for (int k = arr.length; k >= 0; k--){
                                if (arr[k] != 0){
                                    if (arr[k] == this.enrollment.returnEnrollStudent()[i].getCreditsEnrolled() + this.roster.returnRoster()[j].getCredits())
                                        System.out.println(this.roster.returnRoster()[j].toString() + " credits completed: " + arr[k]);
                                }
                            }
                        break;
                    case "Q":
